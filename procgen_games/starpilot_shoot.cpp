...
const std::string NAME = "starpilot_shoot";
...
bool spawn_cmp3(const std::shared_ptr<Entity> &x, const std::shared_ptr<Entity> &y) {
    return (x->spawn_time > y->spawn_time);
}

class StarPilotShootGame : public BasicAbstractGame {
  public:
    ...
    void game_reset() override {
        ...
        std::sort(spawners.begin(), spawners.end(), spawn_cmp3);
    ...
    bool should_fire(const std::shared_ptr<Entity> &e1, int cur_time) {
        ...
        return 0; //cur_time - e1->spawn_time == e1->fire_time;
    }
...
REGISTER_GAME(NAME, StarPilotShootGame);