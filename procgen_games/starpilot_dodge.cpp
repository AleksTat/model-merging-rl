...
const float SURVIVAL_BONUS = 0.1;
...
const int SHOOTER_WIN_TIME = 200;

const int NUM_BASIC_OBJECTS = 9;
const int NUM_SHIP_THEMES = 2;

bool spawn_cmp2(const std::shared_ptr<Entity> &x, const std::shared_ptr<Entity> &y) {
    return (x->spawn_time > y->spawn_time);
}

class StarPilotDodgeGame : public BasicAbstractGame {
  public:
    ...
    void game_reset() override {
        ...
        std::sort(spawners.begin(), spawners.end(), spawn_cmp2);
    ...
    void game_step() override {
        ...
        step_data.reward += SURVIVAL_BONUS;

        bool is_firing = 0; //special_action != 0;
...
REGISTER_GAME(NAME, StarPilotDodgeGame);